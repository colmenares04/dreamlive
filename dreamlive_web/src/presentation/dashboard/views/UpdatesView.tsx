...
const TAG_META: Record<VersionTag, { label: string; icon: string; cls: string }> = {
  new:  { label: 'Novedad',    icon: 'fa-sparkles',      cls: 'bg-blue-100 text-blue-700' },
  fix:  { label: 'Fix',        icon: 'fa-bug',           cls: 'bg-red-100 text-red-700' },
  feat: { label: 'Feature',    icon: 'fa-star',          cls: 'bg-amber-100 text-amber-700' },
  perf: { label: 'Rendimiento',icon: 'fa-bolt',          cls: 'bg-emerald-100 text-emerald-700' },
  sec:  { label: 'Seguridad',  icon: 'fa-shield-halved', cls: 'bg-slate-100 text-slate-700' },
};
...
function UploadBox({ platform, file, onFile }: UploadBoxProps) {
  ...
  return (
    <div
      ...
      />
      <i className={`fab ${isWin ? 'fa-windows text-blue-500' : 'fa-apple text-slate-600'} text-3xl mb-2 block`} />
      <p className="font-bold text-slate-700">{isWin ? 'Windows Build' : 'MacOS Build'}</p>
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-5">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="sm:w-40">
            <label className="block text-xs font-semibold text-slate-600 mb-1.5">Versión (ej: 0.0.6)</label>
            <input
              type="text"
              value={versionNum}
              onChange={e => setVersionNum(e.target.value)}
              placeholder="x.y.z"
              className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-400"
            />
          </div>
          <div className="flex-1">
            <label className="block text-xs font-semibold text-slate-600 mb-1.5">Tipo de Cambios</label>
        ...
        {/* Changelog */}
        <div>
          <label className="block text-xs font-semibold text-slate-600 mb-1.5">Changelog General</label>
          <textarea
            value={changelog}
            onChange={e => setChangelog(e.target.value)}
            rows={2}
            placeholder="Escribe las mejoras generales..."
            className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-400 resize-none"
          />
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100">
          <h3 className="font-semibold text-slate-700">Historial de Versiones</h3>
        </div>
