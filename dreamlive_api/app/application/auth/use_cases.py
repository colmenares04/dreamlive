...
class SelectProfileUseCase:
    ...
    async def execute(self, data: SelectProfileInput) -> SelectProfileOutput:
        ...
        # Si el usuario tiene un hash de contraseña, se requiere validarla
        if user.password_hash:
            if not data.password:
                raise ValueError("Este perfil requiere contraseña.")
            if not PasswordHandler.verify(data.password, user.password_hash):
                raise ValueError("Contraseña de perfil incorrecta.")
