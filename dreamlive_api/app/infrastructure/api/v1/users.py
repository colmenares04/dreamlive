...
audit_router = APIRouter(prefix="/audit-logs", tags=["AuditLogs"])


# ─────────────────────────────────────────────────────────────────────────────
# Users
# ─────────────────────────────────────────────────────────────────────────────
@users_router.get("/", dependencies=[Depends(require_admin_or_programmer)])
async def list_users(agency_id: Optional[str] = None, db: Client = Depends(get_db)):
...
@users_router.post("/", status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_admin_or_programmer)])
async def create_user(body: CreateUserBody, db: Client = Depends(get_db)):
    repo = UserRepository(db)
...
@users_router.patch("/{user_id}", dependencies=[Depends(require_admin_or_programmer)])
async def update_user(user_id: str, body: UpdateUserBody, db: Client = Depends(get_db)):
    repo = UserRepository(db)
...
@users_router.delete("/{user_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_admin_or_programmer)])
...
async def list_tickets(
    ...
    if current_user.role in (UserRole.ADMIN, UserRole.PROGRAMMER):
...
async def create_ticket(body: CreateTicketBody, current_user: User = Depends(get_current_user), db: Client = Depends(get_db)):
    ...
    ticket = Ticket(
        id=None,
        agency_id=body.agency_id or current_user.agency_id or "",
...
async def update_ticket(ticket_id: str, body: UpdateTicketBody, current_user: User = Depends(get_current_user), db: Client = Depends(get_db)):
    ...
    if current_user.role not in (UserRole.ADMIN, UserRole.PROGRAMMER) and ticket.agency_id != current_user.agency_id:
...
# Audit logs
# ─────────────────────────────────────────────────────────────────────────────
@audit_router.get("/", dependencies=[Depends(require_admin_or_programmer)])
...
@audit_router.post("/", status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_admin_or_programmer)])
