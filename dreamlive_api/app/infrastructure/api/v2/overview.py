...
async def get_overview(
    ...
    from app.adapters.db.models import LeadORM, LicenseORM, AgencyORM
    
    res_leads = await uow.session.execute(select(func.count()).select_from(LeadORM))
    total_leads = res_leads.scalar() or 0
    
    res_licenses = await uow.session.execute(select(func.count()).select_from(LicenseORM))
    total_licenses = res_licenses.scalar() or 0
    
    res_agencies = await uow.session.execute(select(func.count()).select_from(AgencyORM))
    total_agencies = res_agencies.scalar() or 0

    return {
        "total_licenses": total_licenses,
        "active_agencies": total_agencies,
        "active_sessions": 0,
        "avg_ticket_sla": 0.0,
