...
router = APIRouter(prefix="/leads", tags=["Leads Web V2"])
...
async def delete_lead(
    ...
    from sqlalchemy import delete
    # Ensure the lead belongs to the agency
    stmt = delete(LeadORM).where(LeadORM.id == lead_id, LeadORM.agency_id == str(agency.id))
    res = await uow.session.execute(stmt)
    await uow.session.flush()
    await uow.session.commit()
    
    if res.rowcount == 0:
        raise HTTPException(status_code=404, detail="Lead no encontrado o no pertenece a tu agencia")
    return {"status": "ok"}
